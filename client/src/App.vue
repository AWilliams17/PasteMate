...
<script>
  ...
  import { RETRIEVE_USER, REFRESH_TOKEN, ADD_NOTIFICATION } from './store/action-types';
      ...
      if (this.$cookies.get('csrf_access_token')) {
        this.$store.dispatch(RETRIEVE_USER).catch(() => {
          this.$store.dispatch(REFRESH_TOKEN)
            .catch(() => {
              this.$store.dispatch(ADD_NOTIFICATION, 'Your session has expired. Please sign back in.');
